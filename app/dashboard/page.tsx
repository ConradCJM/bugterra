  ...
  const getBugsByStatus = (status: string) => {
    return bugs.filter((bug) => {
      ...
      const priorityMatch =
        ...
        selectedPriorities.includes(bug.priority);
      const categoryMatch =
        selectedCategories.length === 0 ||
        selectedCategories.includes(bug.category);
      const searchMatch =
        searchQuery === "" ||
                ...
                {isPriorityDropdownOpen && (
                  <div className="absolute top-full left-0 mt-2 bg-slate-700 rounded-lg shadow-lg border border-slate-600 p-4 z-50 min-w-48">
                    {PRIORITY_FILTERS.map((priority) => (
                      <label
                        key={priority.value}
                        className="flex items-center gap-2 py-2 text-white hover:bg-slate-600 px-2 rounded cursor-pointer"
                      >
                        <input
                          type="checkbox"
              ...
              {(selectedPriorities.length > 0 ||
                ...
                <div className="flex flex-wrap gap-2 items-center">
                  {selectedPriorities.map((priority) => {
                    const priorityFilter = PRIORITY_FILTERS.find(
                      (p) => p.value === priority,
                    );
                    return (
                      <span
                        key={priority}
                        className={`${priorityFilter?.color} text-xs px-2 py-1 rounded-full`}
                      >
                  ...
                  {selectedCategories.map((category) => {
                    const categoryFilter = CATEGORY_FILTERS.find(
                      (c) => c.value === category,
                    );
                    return (
                      <span
                        key={category}
                        className={`${categoryFilter?.color} text-xs px-2 py-1 rounded-full`}
                      >
                        {category}
                      </span>
                    );
